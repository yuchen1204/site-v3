...
async function loadCommentsForAdmin() {
    const accordionContainer = document.getElementById('comment-accordion');
    if (!accordionContainer) {
        console.error('Comment accordion container not found!');
        return;
    }
    ...
    try {
        ...
        const postIds = Object.keys(commentsByPost);

        if (postIds.length === 0) {
            accordionContainer.innerHTML = '<p class="text-muted">目前还没有任何评论。</p>';
            return;
        }
        ...
        // 按 Post ID 排序显示 (可选)
        postIds.sort((a, b) => parseInt(a, 10) - parseInt(b, 10));

        postIds.forEach(postId => {
            const comments = commentsByPost[postId];
            const accordionItem = document.createElement('div');
            accordionItem.className = 'accordion-item';
            accordionItem.id = `accordion-item-post-${postId}`;

            const commentsHtml = comments.map(comment => {
                const commentDate = new Date(comment.timestamp);
                const formattedDate = commentDate.toLocaleString('zh-CN');
                const gravatarSize = 30;
                ...
                const gravatarUrl = comment.emailHash
                    ...
                    : `https://www.gravatar.com/avatar/?s=${gravatarSize}&d=${encodeURIComponent(gravatarDefault)}`;

                return `
                    <li class="list-group-item d-flex justify-content-between align-items-start" id="comment-${comment.id}">
                        <div class="ms-2 me-auto">
                            <div class="fw-bold">
                                <img src="${gravatarUrl}" alt="" width="${gravatarSize}" height="${gravatarSize}" class="rounded-circle me-2">
                                ${escapeHtml(comment.author)}
                            </div>
                            <p class="mb-1 ms-4 ps-3">${escapeHtml(comment.text)}</p>
                            <small class="text-muted ms-4 ps-3">${formattedDate}</small>
                        </div>
            ...
            accordionItem.innerHTML = `
                <h2 class="accordion-header" id="heading-post-${postId}">
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-post-${postId}" aria-expanded="false" aria-controls="collapse-post-${postId}">
                        文章 ID: ${postId} <span class="badge bg-secondary ms-2">${comments.length} 条评论</span>
                    </button>
                </h2>
                <div id="collapse-post-${postId}" class="accordion-collapse collapse" aria-labelledby="heading-post-${postId}" data-bs-parent="#comment-accordion">
        ...
        accordionContainer.innerHTML = '<p class="text-danger">加载评论失败，请检查网络连接或稍后重试。</p>';
    }
}
...
async function handleDeleteCommentClick(event) {
    ...
    if (!postId || !commentId) {
        console.error('Missing postId or commentId on delete button');
        return;
    }
    ...
    if (confirm(`确定要删除这条评论吗？此操作不可撤销。`)) {
        ...
        try {
            // TODO: 添加认证头（如果需要）
            const response = await fetch(`/admin/api/comments?postId=${postId}&commentId=${commentId}`, {
                method: 'DELETE'
            });
            ...
            if (response.ok && result.success) {
                ...
                if (commentElement) {
                    const list = commentElement.closest('ul');
                    commentElement.remove();
                    // 检查列表是否为空，如果为空，可以考虑移除整个 accordion item 或更新徽章
                    if (list && list.children.length === 0) {
                        const accordionItem = document.getElementById(`accordion-item-post-${postId}`);
                        if (accordionItem) {
                            // 可选：移除整个文章的评论区
                            // accordionItem.remove(); 
                            // 或者更新标题
                            const headerButton = accordionItem.querySelector('.accordion-button');
                            if(headerButton) headerButton.innerHTML = `文章 ID: ${postId} <span class="badge bg-light text-dark ms-2">0 条评论</span>`;
                        }
                    } else {
                         // 更新徽章计数
                         const headerButton = document.querySelector(`#heading-post-${postId} .accordion-button`);
                         if(headerButton && list) {
                             headerButton.querySelector('.badge').textContent = `${list.children.length} 条评论`;
                         }
                    }
                } 
                alert('评论已删除。'); // 简单提示
            ...
            // 恢复按钮状态 (如果元素未被移除)
             const currentButton = document.querySelector(`.delete-comment-btn[data-comment-id="${commentId}"]`);
            if (currentButton) {
                 currentButton.disabled = false;
                 currentButton.innerHTML = '<i class="bi bi-trash"></i>';
             }
        }
    }
}
...
// 可以在这里添加其他后台管理功能相关的JS，例如 checkAuth, loadPostsForAdmin 等 